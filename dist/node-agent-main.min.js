...
var TURNED_ON_FILE_NAME = "TURNED_ON_OK.txt";
...
var bootstrap = () => {
  ...
  const hasStarted = fs.existsSync(import_path.default.resolve(TURNED_ON_FILE_NAME));
  const hasGracefulShutdown = fs.existsSync(import_path.default.resolve(TURNED_OFF_FILE_NAME));
  try {
    console.log("Attempting to delete old bootstrap files");
    if (hasStarted) {
      fs.unlinkSync(import_path.default.resolve(TURNED_ON_FILE_NAME));
    }
    if (hasGracefulShutdown) {
      fs.unlinkSync(import_path.default.resolve(TURNED_OFF_FILE_NAME));
    }
  } catch (error) {
    console.log("ERROR deleting old bootstrap files");
    console.error(error);
  } finally {
    fs.writeFileSync(import_path.default.resolve(TURNED_ON_FILE_NAME), "", "ascii");
    console.groupEnd();
  }
};
var readStorage = () => {
  console.group("ReadStorage fn");
  const storage = fs.existsSync(import_path.default.resolve(CONFIG_FILE_NAME)) && JSON.parse(fs.readFileSync(import_path.default.resolve(CONFIG_FILE_NAME), "ascii"));
  if (Boolean(storage)) {
...
var setStorage = (payload) => {
  console.group("SetStorage fn");
  console.log("Attempting to write config file to storage");
  fs.writeFileSync(import_path.default.resolve(CONFIG_FILE_NAME), JSON.stringify(payload), "ascii");
  console.groupEnd();
};
var updateStorage = async (payload) => {
...
var handleCommand = async (command) => {
  ...
  try {
    switch (command) {
      case "restart":
        console.log("attempting device restart");
        return;
      default:
        console.log("command handling: default case");
        return;
    }
  } catch (error) {
    throw error;
...
var requestAPI = async (url, requestPayload) => {
  ...
  if (rawResponse.status === 401 || rawResponse.status === 403) {
    ...
    try {
      ...
      fs2.unlinkSync(import_path2.default.resolve(CONFIG_FILE_NAME));
    } catch (error) {
...
var notifyServerLoop = async (deviceId, accessKey) => {
  ...
  if (Boolean(commandFlag)) {
    ...
    await network_default(`${storedConfig.hub_url}/v1/devices/${deviceId}/command`, {
      method: "POST",
      headers: {
        "Authorization": accessKey,
        "Content-Type": "application/json",
...
process.on("SIGTERM", () => {
  ...
  import_fs.default.writeFileSync(import_path3.default.resolve(TURNED_OFF_FILE_NAME), "", "ascii");
  console.groupEnd();
  setTimeout(() => {
    process.exit(0);
  }, INTERVAL_IN_MS).unref();
});
async function main() {
  console.log("-----------------------------------------------------------------------------------------");
