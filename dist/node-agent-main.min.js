...
var HARDWARE_KEY = "c3b35b65-0e01-454e-a82d-aa322057a8ca";
var NANO_ID = "WleMAojHpC7Ufmtoy-MiLTRa";
var FIRMWARE_VERSION = "1.0.0";
var CONFIG_FILE_NAME = "config.txt";
var FIRMWARE_FILE_NAME = "firmware.txt";
var COMMAND_FILE_NAME = "command-log.txt";
var STD_LOG_FILE = "out.log";
var ERR_LOG_FILE = "err.log";
var TURNED_OFF_FILE_NAME = "TURNED_OFF_OK.txt";
var DEVICE_REGISTRATION_SERVER = "https://entry.xyte.io";
var DEVICE_REGISTRATION_PROXY = "http://proxy.xyte.local";
var INTERVAL_IN_MS = 10 * 1e3;

// src/helpers/storage.ts
var import_fs = __toESM(require("fs"), 1);
var import_path = __toESM(require("path"), 1);
var setShutdownToStorage = () => import_fs.default.writeFileSync(import_path.default.resolve(TURNED_OFF_FILE_NAME), "", "ascii");
var removeShutdownFromStorage = () => import_fs.default.unlinkSync(import_path.default.resolve(TURNED_OFF_FILE_NAME));
var hasGracefulShutdown = () => Boolean(import_fs.default.existsSync(import_path.default.resolve(TURNED_OFF_FILE_NAME)));
var setFirmwareToStorage = (payload) => import_fs.default.writeFileSync(import_path.default.resolve(FIRMWARE_FILE_NAME), JSON.stringify(payload), "ascii");
var readStdLogFromStorage = () => Boolean(import_fs.default.existsSync(import_path.default.resolve(STD_LOG_FILE))) && import_fs.default.readFileSync(import_path.default.resolve(STD_LOG_FILE), "ascii");
var readErrLogFromStorage = () => Boolean(import_fs.default.existsSync(import_path.default.resolve(ERR_LOG_FILE))) && import_fs.default.readFileSync(import_path.default.resolve(ERR_LOG_FILE), "ascii");
var logCommandToStorage = (commandInfo) => {
  console.group("logCommandToStorage fn");
  console.log("Attempting to log command info to file storage");
  import_fs.default.writeFileSync(import_path.default.resolve(COMMAND_FILE_NAME), JSON.stringify(commandInfo), "ascii");
  console.groupEnd();
};
var clearCommandLogFromStorage = () => {
  console.group("clearCommandLogFromStorage fn");
  console.log("Attempting to clear log command from file storage");
  Boolean(import_fs.default.existsSync(import_path.default.resolve(COMMAND_FILE_NAME))) && import_fs.default.unlinkSync(import_path.default.resolve(COMMAND_FILE_NAME));
  console.groupEnd();
};
var getCommandFromStorage = () => Boolean(import_fs.default.existsSync(import_path.default.resolve(COMMAND_FILE_NAME))) && JSON.parse(import_fs.default.readFileSync(import_path.default.resolve(CONFIG_FILE_NAME), "ascii"));
var readConfigFromStorage = () => {
  console.group("readConfigFromStorage fn");
  const storage = Boolean(import_fs.default.existsSync(import_path.default.resolve(CONFIG_FILE_NAME))) && JSON.parse(import_fs.default.readFileSync(import_path.default.resolve(CONFIG_FILE_NAME), "ascii"));
  if (Boolean(storage)) {
...
var setConfigToStorage = (payload) => {
  console.group("setConfigToStorage fn");
  console.log("Attempting to write config file to storage");
  import_fs.default.writeFileSync(import_path.default.resolve(CONFIG_FILE_NAME), JSON.stringify(payload), "ascii");
  console.groupEnd();
};
var updateConfigInStorage = async (payload) => {
...
var authenticateDeviceFromStorage = () => {
...
var import_compare_versions = __toESM(require_compare_versions(), 1);
var applyLicense = async (license) => {
...
var removeLicense = async (license) => {
  ...
  try {
    switch (command.name) {
      case "update_firmware":
        ...
        const serverFirmwareVersion = ((_a = command == null ? void 0 : command.parameters) == null ? void 0 : _a.version) || FIRMWARE_VERSION;
        const deviceFirmwareVersion = FIRMWARE_VERSION;
        if ((0, import_compare_versions.default)(serverFirmwareVersion, deviceFirmwareVersion)) {
          const firmwareUrl = command.parameters.url;
          console.log("this is a simple GET to any url, you may want to use Xyte GetFile API");
          const firmwareFile = await network_default(firmwareUrl, {
            method: "GET",
            headers: {
          ...
          const hasFirmwareInStorage = (0, import_child_process.execSync)("ls").toString().includes(FIRMWARE_FILE_NAME);
          return {
            id: command.id,
      ...
      case "restart":
        ...
        console.log((0, import_child_process.execSync)("/sbin/shutdown -r now"));
        console.log("device should restart immediately, if we've reached this point it didn't");
        return {
          id: command.id,
          status: "failed",
          message: "Unable To restart device"
        };
      case "dump":
        ...
        if (errorLogDump) {
          const dumpId = await network_default(`${storedConfig.hub_url}/v1/devices/${storedConfig.id}/dumps/text%2Ftxt/agent.log`, {
            method: "POST",
            headers: {
              "Authorization": storedConfig.access_key,
          ...
          standardLogDump && await network_default(`${storedConfig.hub_url}/v1/devices/${storedConfig.id}/dumps/${dumpId}`, {
            method: "PUT",
            headers: {
              "Authorization": storedConfig.access_key,
              "Content-Type": "text/plain",
...
// src/helpers/network.ts
var requestAPI = async (url, requestPayload) => {
  ...
  if (rawResponse.status === 401 || rawResponse.status === 403) {
    ...
    try {
      ...
      import_fs2.default.unlinkSync(import_path2.default.resolve(CONFIG_FILE_NAME));
    } catch (error) {
...
var network_default = requestAPI;

// src/authentication.ts
var REGISTRATION_PAYLOAD = JSON.stringify({
  nano_id: NANO_ID,
...
var registerDevice = async () => {
  ...
  try {
    registrationResponse = await network_default(`${DEVICE_REGISTRATION_SERVER}/v1/devices`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
    ...
    registrationResponse = await network_default(`${DEVICE_REGISTRATION_PROXY}/v1/devices`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Content-Length": `${REGISTRATION_PAYLOAD.length}`
...
var authentication_default = authenticateDevice;

// src/config.ts
var evaluateConfigVersion = async (deviceId, accessKey, serverVersion) => {
  ...
  if (serverVersion > localVersion) {
    const newConfig = await network_default(`${storedConfig.hub_url}/v1/devices/${deviceId}/config`, {
      method: "GET",
      headers: {
    ...
    await network_default(`${storedConfig.hub_url}/v1/devices/${deviceId}/config`, {
      method: "POST",
      headers: {
        "Authorization": accessKey,
        "Content-Type": "application/json",
...
var config_default = evaluateConfigVersion;

// src/command.ts
var import_os = __toESM(require("os"), 1);
var checkSavedCommandAfterBoot = async (command) => {
  const hasFreshBooted = import_os.default.uptime() <= 60 * 5;
  if (hasFreshBooted && command) {
    ...
    await network_default(`${storedConfig.hub_url}/v1/devices/${storedConfig.id}/command`, {
      method: "POST",
      headers: {
        "Authorization": storedConfig.access_key,
        "Content-Type": "application/json",
...
var handleCommand = async (command) => {
  console.group("handleCommand fn");
...
var command_default = handleCommand;

// src/licenses.ts
var handleAddLicense = async (deviceId, accessKey, license) => {
  ...
  await network_default(`${config.hub_url}/v1/devices/${deviceId}/licenses`, {
    method: "POST",
    headers: {
      "Authorization": accessKey,
      "Content-Type": "application/json",
...
var handleRemoveLicense = async (deviceId, accessKey, license) => {
  console.group("HandleRemoveLicense fn");
  await removeLicense(license);
  const config = readConfigFromStorage();
  const newLicensesList = (config && config.licenses || []).filter((item) => item.id !== license.id);
...
var handleLicense = async (deviceId, accessKey, license) => {
...
var licenses_default = handleLicense;

// src/scheduler.ts
var notifyServerLoop = async (deviceId, accessKey) => {
  console.log();
  ...
  } = await network_default(`${storedConfig.hub_url}/v1/devices/${deviceId}/telemetry`, {
    method: "POST",
    headers: {
      "Authorization": accessKey,
      "Content-Type": "application/json",
  ...
  await config_default(deviceId, accessKey, configVersion);
  if (Boolean(commandFlag)) {
    const command = await network_default(`${storedConfig.hub_url}/v1/devices/${deviceId}/command`, {
      method: "GET",
      headers: {
        "Authorization": accessKey,
        "Content-Type": "application/json"
    ...
    await command_default(command);
  }
  if (Boolean(newLicenses)) {
    const licenses = await network_default(`${storedConfig.hub_url}/v1/devices/${deviceId}/licenses`, {
      method: "GET",
      headers: {
        "Authorization": accessKey,
        "Content-Type": "application/json"
    ...
    for (const license of licenses) {
      await licenses_default(deviceId, accessKey, license);
    }
  }
  console.groupEnd();
  setTimeout(async () => await notifyServerLoop(deviceId, accessKey), INTERVAL_IN_MS);
};
var scheduler_default = notifyServerLoop;
...
async function main() {
  ...
  try {
    const authConfig = await authentication_default();
    if (!authConfig) {
      throw new Error("Critical Error: missing device auth configuration. unable to proceed. exiting!");
    }
    await scheduler_default(authConfig.id, authConfig.access_key);
  } catch (error) {
    console.error(error);
